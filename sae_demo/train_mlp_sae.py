...
def get_args():
    ...
    parser.add_argument(
        ...
        help="whether to extract input or output activations",
    )

    parser.add_argument('--num_tokens', type=int, default=50_000_000,
                        help='Number of tokens to train on. Default is 50_000_000.')
...
def run_sae_training(
    ...
    model.eval()
    print('Model truncated to layer:', layer)
    print(model)

    tokenizer = AutoTokenizer.from_pretrained(model_name)
    print('Submodule name:', model_name)
    submodule = utils.get_submodule(model, layer).mlp
    print('Submodule:', submodule)
    io = args.io
    submodule_name = f"mlp_{io}_{layer}"
...
def eval_saes(
    ...
    for ae_path in ae_paths:
        ...
        assert(False), "Not implemented -- this is not using MLP"
