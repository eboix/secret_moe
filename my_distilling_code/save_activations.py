...
def save_activations_for_split(model, tokenizer, layer_idx, dataset_name, dataset_config, split_name,
                               ...
                               max_text_samples=None, min_token_length=20): # MODIFIED: Added min_token_length
    """
    ...
    output_filename = f"{s_dataset}_{s_config}_{s_model}_layer{layer_idx}_act{activation_type}_{split_name}_mintok{min_token_length}.pt"
    output_path = os.path.join(output_dir, output_filename)
    os.makedirs(output_dir, exist_ok=True)

    print(f"Collecting activations from layer {layer_idx} ({activation_type}). Target file: {output_path}")
    
    progress_bar = tqdm(desc=f"Saving to {output_filename}", unit=" act_batch")
    ...
    if not all_activations_list:
        print(f"No activations collected for {split_name}. Skipping save for {output_path}.")
        return
    ...
    try:
        ...
        torch.save(final_activations_tensor, output_path)
        print(f"Successfully saved {split_name} activations to {output_path}")
    except Exception as e:
        print(f"Error concatenating or saving {split_name} activations: {e}")
        if all_activations_list: print(f"Shape of first collected batch: {all_activations_list[0].shape}")
