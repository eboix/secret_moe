...
def parse_args():
    ...
    parser.add_argument('--key_proj_dim', type=int, default=128,
                        help='Dimension of the key projection for MoE')
...
class MoE_TopK_SingleNeuron(nn.Module):
    def __init__(self, input_dim, output_dim, num_experts, k, hidden_dim=1, key_proj_dim=128, bias=False):
        super().__init__()
        ...
        self.key_proj_dim = key_proj_dim
        self.key_proj = nn.Linear(input_dim, key_proj_dim, bias=False)

        # Gating network
        self.gate = nn.Linear(key_proj_dim, num_experts, bias=bias)
        ...
        self.weight_down_embed = nn.Embedding(num_experts, input_dim)
        self.weight_up_embed = nn.Embedding(num_experts, output_dim)
        self.down_bias = nn.Embedding(num_experts, 1)
        self.up_bias = nn.Parameter(torch.zeros(output_dim))

        torch.nn.init.uniform_(self.weight_down_embed.weight, a=-1/math.sqrt(input_dim), b=1/math.sqrt(input_dim))
        # torch.nn.init.uniform_(self.weight_up_embed.weight, a=-1/math.sqrt(output_dim), b=1/math.sqrt(output_dim)) # it would be better to use granularity maybe?
        torch.nn.init.uniform_(self.weight_up_embed.weight, a=-1/math.sqrt(k), b=1/math.sqrt(k)) # it would be better to use granularity maybe?
        torch.nn.init.uniform_(self.down_bias.weight, a=-1/math.sqrt(input_dim), b=1/math.sqrt(input_dim))
        torch.nn.init.zeros_(self.up_bias)

        # self.expert_fc2 = nn.Linear(num_experts, output_dim, bias=bias)

    def forward(self, x):
        gate_scores = self.gate(self.key_proj(x))  # (batch_size, num_experts) 
        ...
        down_bias = self.down_bias(topk_idxs).squeeze(-1)  # (batch_size, k)
        weights_down = self.weight_down_embed(topk_idxs)  # (batch_size, k, input_dim)
        x = torch.einsum('bd,bkd->bk', x, weights_down)  # (batch_size, k)
        # free weights_down memory
        del weights_down
        x = x + down_bias  # (batch_size, k)
        del down_bias

        x = torch.nn.functional.gelu(x)  # Apply activation

        x = x * topk_weights  # (batch_size, k)
        del topk_weights

        weights_up = self.weight_up_embed(topk_idxs)  # (batch_size, k, output_dim)
        x = torch.einsum('bk,bkd->bd', x, weights_up)  # (batch_size, output_dim)
        del weights_up

        x = x + self.up_bias  # (batch_size, output_dim)
        return x
...
class MLPPlusMoE(nn.Module):
    def __init__(self, input_dim, output_dim, shared_hidden_dim, num_experts, expert_hidden_dim, granularity, key_proj_dim, dtype=torch.float32):
        super().__init__()
        if shared_hidden_dim > 0:
            self.shared_mlp = MLP(input_dim, output_dim, hidden_dim=shared_hidden_dim, dtype=dtype)
        else:
            self.shared_mlp = None
        self.moe = MoE_TopK_SingleNeuron(input_dim, output_dim, num_experts=num_experts, k=granularity, hidden_dim=expert_hidden_dim, key_proj_dim=key_proj_dim, bias=True)
...
student_model = MLPPlusMoE(input_dim=d_in, output_dim=d_out,
                           ...
                           expert_hidden_dim=args.expert_hidden_dim, granularity=args.granularity, key_proj_dim=args.key_proj_dim, dtype=torch_dtype).to(device)
print(student_model)
student_model = torch.compile(student_model, mode='default', fullgraph=True, dynamic=True)
